...
export async function generateAndEmailPlan(body: any) {
  try {
...
- Use encouraging but realistic tone`
    ...
    if (needsRoadmap) {
      ...
      (async () => {
        try {
          const { generateRoadmap } = await import("./roadmapService");
          const roadmap = await generateRoadmap({ formData, month1Plan: aiPlan, timeline: formData.timeline });

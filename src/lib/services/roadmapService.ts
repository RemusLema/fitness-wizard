...
export async function generateRoadmap({ formData, month1Plan, timeline }: { formData: any; month1Plan: any; timeline: string }) {
    try {
        if (!formData || !timeline) {
            throw new Error("Missing formData or timeline");
        }
